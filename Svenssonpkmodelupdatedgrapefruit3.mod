$PROBLEM    TMC207
;; 1. Based on: SvenssonpkmodelupdatedgrapefruitCMT8
;; 2. Description: TMC207   TMC207   Svensson pk model - approved dose
;; x1. Author: peter
$INPUT      ID WOMEN RACE DV TIME EVID CMT MDV AMT STUD WT ALB AGE OCC
            FLAG
$DATA      COSVEN4_B300.csv IGNORE=@
$SUBROUTINE ADVAN13 TOL=6
$MODEL      NCOMPARTMENTS=9 COMP=(DEPOT DEFDOSE) COMP=(BDQC)
            COMP=(BDQPERI1) COMP=(BDQPERI2) COMP=(M2) COMP=(TRANSI1)
            COMP=(TRANSI2) COMP=(GRAPE) COMP=(ALBUMIN)
$PK
;--- Model for Albumin
TVX0 = THETA(1)
TVXSS = THETA(2)
...
$DES
; --- Albumin
DADT(9) = LOG(2)/(HL*7*24)*A(9)*(1- A(9)/XSS) ; Time in hours, unit of half life: weeks
; --- Body weight
...
BDQ=A(2)
GRAPE_CMT = A(8)
IC50=0.586+0.000001 ; conc is the average of BDQ
 
IF (MYTIME.EQ.0) THEN
INH=1 
ELSE
INH=RCLF+(1-RCLF)*(1-(1*GRAPE_CMT+0.00001)/(IC50+GRAPE_CMT+0.000001))
ENDIF
 
CL = INH*CLB*COVALBI*ALLCL
V = VB*ALLV
...
Q2 = Q2B*ALLCL
VP2 = VP2B*ALLV
CLM2 = INH*CLM2B/FM*COVALBI*ALLCL
VM2 = VM2B/FM*ALLV
 
DADT(1)= -KTR*A(1)
DADT(2) = A(7)*KA - CL*A(2)/V- A(2)*Q1/V + A(3)*Q1/VP1 - A(2)*Q2/V + A(4)*Q2/VP2 
...
DADT(8) = KIN*(CP - A(8)) ; inhibition compartment
...
;
;$ESTIMATION MAXEVAL=2000 METHOD=1 INTER NOABORT NSIG=1
;$EST METHOD=IMP INTERACTION EONLY=1 NITER=5 ISAMPLE=1000 PRINT=1
;$COVARIANCE MATRIX=R UNCOND PRINT=E
$TABLE ID TIME IPRED CWRES CP GRAPE_CMT INH MDV CMT CL BDQ FILE=sdtab12grapefruit NOPRINT